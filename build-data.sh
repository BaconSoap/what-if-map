...
ELECTORAL_VOTES_URL='https://raw.githubusercontent.com/BaconSoap/datasets/master/election-2016/electoral-votes-per-state.csv'
...
curl $ELECTORAL_VOTES_URL >> 2016_electoral_votes.csv
...
$MAPSHAPER data.shp -simplify 10% \
  ...
  -join 2016_electoral_votes.csv keys=state_abbr,state_abbr \
  -filter-fields "total_votes_2012,total_votes_2016,votes_dem_2012,votes_dem_2016,votes_gop_2012,votes_gop_2016,county_name,state_abbr,POP_ESTIMATE_2015,STATEFP,electoral_votes" \
  -o counties.json force id-field=GEOID format=geojson

$MAPSHAPER counties.json \
  -dissolve STATEFP sum-fields='POP_ESTIMATE_2015' copy-fields='state_abbr,electoral_votes' \
  -o states.json force id-field=STATEFP format=geojson

rm data.dbf data.shp source.zip all_results.csv 2015_population.csv 2016_electoral_votes.csv
